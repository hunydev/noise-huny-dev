...
function makeGaussian(rng: RNG): () => number {
...
// ========== Noise Generation ==========
function generateNoise({ N, rmsDBFS, softenEdges, seed }: { N: number; rmsDBFS: number; softenEdges: boolean; seed?: string | number | null; }): Float32Array {
...
function makeWavBlob({ samples, sampleRate }: { samples: Float32Array; sampleRate: number; }): Blob {
...
// ========== Waveform Drawing ==========
function drawWaveform(canvas: HTMLCanvasElement | null, samples: Float32Array | null, { normalize }: { normalize: boolean }) {
  if (!canvas) return;
...
function runSelfTests(): TestResult[] {
  ...
  function t(name: string, fn: () => void) {
    ...
    catch (e: any) { results.push({ name, pass: false, err: String(e) }); }
  }
...
export default function EndpointConstrainedNoiseApp() {
  ...
  const [metrics, setMetrics] = useState<{ N: number; rms: number; rmsDB: number; first: number; last: number }>({ N: 0, rms: 0, rmsDB: -Infinity, first: 0, last: 0 });
  ...
  const [pcmName, setPcmName] = useState<string>("");
